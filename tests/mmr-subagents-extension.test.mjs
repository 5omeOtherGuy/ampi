...
describe("mmr-subagents package wiring", () => {
  ...
  it("re-exports the mmr-subagents public surface from the package root", async () => {
    ...
    assert.equal(typeof root.createReviewerTool, "function");
    assert.equal(typeof root.registerReviewerTool, "function");
    assert.equal(typeof root.buildReviewerWorkerSystemPrompt, "function");
    assert.equal(root.REVIEWER_TOOL_NAME, "reviewer");
    assert.equal(root.REVIEWER_SUBAGENT_PROFILE, "reviewer");
    assert.deepEqual([...root.REVIEWER_WORKER_TOOLS], ["read", "grep", "find", "bash"]);
