...
const asyncTasksExtensionPath = "./src/extensions/ampi-workers/index.ts";
...
describe("mmr-workers merged extension", () => {
  it("is registered as a Pi package extension after mmr-core", async () => {
    ...
    assert.equal(pkg.pi.extensions.indexOf("./src/extensions/ampi-async-tasks/index.ts"), -1, "the pre-merge entrypoint is removed");
    assert.equal(pkg.pi.extensions.indexOf("./src/extensions/mmr-async-tasks/index.ts"), -1, "the legacy pre-merge entrypoint stays removed");
  });

  it("exposes a package subpath for direct extension loading", async () => {
    const pkg = await readPackageJson();
    assert.equal(pkg.exports["./extensions/ampi-workers"], asyncTasksExtensionPath);
    assert.equal(pkg.exports["./extensions/ampi-async-tasks"], undefined, "the pre-merge subpath is removed");
    assert.equal(pkg.exports["./extensions/mmr-async-tasks"], undefined, "the legacy pre-merge subpath stays removed");
  });
