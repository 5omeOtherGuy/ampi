...
describe("mmr-web URL policy", () => {
  it("accepts a normal public https URL", async () => {
    const { validateExternalHttpUrl } = await importSource("extensions/mmr-web/url-policy.ts");
...
describe("mmr-web SearXNG URL validation", () => {
  it("accepts loopback / private hosts because the URL is user-trusted", async () => {
    const { validateSearXNGUrl } = await importSource("extensions/mmr-web/url-policy.ts");
    for (const url of [
      "http://127.0.0.1:8080",
      "http://localhost:8080",
      "http://10.0.0.5:8080",
