...
describe("background-agent derivation", () => {
  it("derives the built-in agent set from backgroundable profiles, default agent first", async () => {
    ...
    assert.deepEqual(names, ["Task", "finder", "reviewer", "librarian"]);
  });
  ...
  it("appends a registered custom agent to the derived schema and description", async () => {
    ...
    assert.deepEqual(literals, ["Task", "finder", "reviewer", "librarian", "sa__probe"]);
    ...
    assert.match(schemas.buildStartTaskDescription(), /Task \(default\), finder, reviewer, librarian, or sa__probe/);
  });
  ...
  it("keeps the module-load schema snapshot byte-stable for the built-in set", async () => {
    ...
    assert.deepEqual(literals, ["Task", "finder", "reviewer", "librarian"]);
    assert.match(
      schemas.START_TASK_PARAMETERS.properties.agent.description,
      /Task \{prompt,description\}, finder \{query\}, reviewer \{diff_description,files\?,instructions\?\}, librarian \{query,context\?\}/,
    );
  });
});
