...
describe("assembleActiveSurface() prompt-tail drift hardening", () => {
  ...
  for (const mode of PROMPTED_MODES) {
    it(`preserves new system prompt collaboration/tool ordering for ${mode}`, () => {
      const first = assembleActiveSurface({
      ...
      if (mode === "low") {
        ...
        assertBefore(first.systemPrompt, collaborationHeading, TOOL_USE_HEADING, `${mode} fresh prompt`);
      ...
      assert.equal(second.systemPrompt, first.systemPrompt, `${mode}: prompt must re-assemble byte-stably`);
      assert.equal(second.passthroughReason, undefined, `${mode}: re-assembly must remain a real rewrite`);
      assert.equal(second.systemPrompt.split(collaborationHeading).length - 1, 1, `${mode}: collaboration heading must not duplicate`);
