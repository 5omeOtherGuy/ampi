# Per-user data storage conventions for `ampi` extensions
...
```text
<getAgentDir()>/data/ampi/<feature>/<scope-key>...
```
...
- `data/ampi/` is the fixed root for all `ampi` package state. Keeping the package name in the path means an unrelated extension installed under the same Pi agent dir cannot collide with us, and a future package rename can grep for one directory.
...
- Resolver: `resolveDefaultTaskStoreBaseDir()` returned `path.join(getAgentDir(), "data", "ampi", "task-list")`.
...
- Final path: `~/.pi/agent/data/ampi/task-list/<sha256(workspaceRoot)>.json` by default; `$PI_CODING_AGENT_DIR/data/ampi/task-list/...` when the env var is set.
...
- [ ] Path under `<getAgentDir()>/data/ampi/<feature>/...` (imported via `getAgentDir`, not reconstructed).
