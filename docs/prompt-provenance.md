...
- `smart`, `smartGPT`, `rush`, `large`, and `deep` each use a pi-mmr-authored mode template (intro, posture sections, closing line) in `src/extensions/mmr-core/prompt-content.ts` (re-exported by the `prompt-templates.ts` compatibility shim).
...
- For each non-`free` turn, `mmr-core` surgically replaces only Pi's auto-rendered head (identity line through the `Pi documentation` block) with the active mode prompt.
