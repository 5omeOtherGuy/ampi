...
- `smart`, `smartGPT`, `rush`, `large`, and `deep` each use a pi-mmr-authored mode template (intro, posture sections, closing line) in `src/extensions/mmr-core/prompt-templates.ts`.
