...
- `smart`, `smartFable`, `rush`, and `deep` each use a pi-mmr-authored mode template (intro, posture sections, closing line) in `src/extensions/mmr-core/prompt-content.ts` (re-exported by the `prompt-templates.ts` compatibility shim).
