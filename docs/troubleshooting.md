...
| `rate_limit` / `overloaded_error` / `upstream_capacity_signal=silent_200_stream` | `mmr-session-fallback` override entries and model changes after the error | Provider capacity/quota; ampi may apply an interactive fallback only after classification |
