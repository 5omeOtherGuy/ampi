...
Audience: ampi users and maintainers diagnosing locked-mode, provider, and tool-call failures.

Scope: symptoms visible in Pi session logs and ampi diagnostics; provider internals and credentials stay out of this repository.

Related: package overview ([`README.md`](../README.md)), core diagnostics ([`src/extensions/ampi-core/README.md`](../src/extensions/ampi-core/README.md#diagnostics-and-troubleshooting)), session fallback ([`src/extensions/ampi-session-fallback/README.md`](../src/extensions/ampi-session-fallback/README.md)), and history tools ([`src/extensions/ampi-history/README.md`](../src/extensions/ampi-history/README.md)).
...
| `rate_limit` / `overloaded_error` / `upstream_capacity_signal=silent_200_stream` | `ampi-session-fallback` override entries and model changes after the error | Provider capacity/quota; ampi may apply an interactive fallback only after classification |
...
3. Attribute the failure by evidence:
   ...
   - If the error is `rate_limit`, `overloaded_error`, `upstream_capacity_signal=silent_200_stream`, or transport-only, treat it as provider capacity/transport unless a ampi fallback or request-policy rewrite is visible in the same turn.
4. Check ampi state only after the stream facts:
   - `ampi-core.mode-state` shows the active mode, resolved provider/model, thinking level, and fallback status.
   - `ampi-session-fallback.override` shows interactive fallback selections after classified quota/rate-limit failures.
   - `/ampi-status debug` explains model/tool resolution; it does not prove a provider stream was valid.
