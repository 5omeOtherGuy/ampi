...
#   - the `prepare` npm script's drift hint (non-fatal)
#   - the git pre-commit / pre-push hooks (fatal)
#   - any agent preflight that wants to verify primary freshness without
#     mutating anything
#
...
if [ "$AHEAD" -gt 0 ] && [ "$BEHIND" -gt 0 ]; then
  ...
  emit "  run 'scripts/sync-primary.sh' after reconciling, or fix manually"
  exit 31
fi

if [ "$BEHIND" -gt 0 ]; then
  emit "check-primary-fresh: primary main is BEHIND origin/main by $BEHIND commit(s)"
  emit "  run 'scripts/sync-primary.sh' to fast-forward (working tree must be clean)"
  exit 30
fi
