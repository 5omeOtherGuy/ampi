...
async function pickModes(ctx: ExtensionContext): Promise<MmrCustomSubagentModeScope | undefined> {
  ...
  const raw = await ctx.ui.input("Modes (comma-separated: smart, smartGPT, rush, large, deep)", "deep");
  if (raw === undefined) return undefined;
  const modes = parseModeList(raw);
  if (!modes) {
    ctx.ui.notify("No valid locked modes entered; expected smart, smartGPT, rush, large, or deep.", "error");
    return undefined;
  }
  return modes;
}
