...
import { registerAmpiGithubToolSourcePath } from "./tool-ownership.js";
...
// entrypoint path here lets `ampi-core` Free mode and worker librarian
...
// preserved.
const AMPI_GITHUB_EXTENSION_ENTRYPOINT_PATH = fileURLToPath(import.meta.url);
registerAmpiOwnedExtensionPath(AMPI_GITHUB_EXTENSION_ENTRYPOINT_PATH);
registerAmpiGithubToolSourcePath(AMPI_GITHUB_EXTENSION_ENTRYPOINT_PATH);
...
/**
 * Build a Pi extension factory for `ampi-github` with optional test seams.
 *
 ...
 *
 * Settings are sampled once at extension load, matching `ampi-web`: the Pi
...
export function createMmrGithubExtension(overrides: MmrGithubFactoryOverrides = {}) {
  return function mmrGithubExtension(pi: ExtensionAPI): void {
    ...
    registerMmrGithubTools(pi, deps, AMPI_GITHUB_EXTENSION_ENTRYPOINT_PATH);
...
export const createAmpiGithubExtension = createMmrGithubExtension;

const ampiGithubExtension = createMmrGithubExtension();

export default ampiGithubExtension;
