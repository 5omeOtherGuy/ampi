 ...
 * per call (timeout + byte caps via the shared `ampi-web` HTTP helpers) and
...
function mapErrorResponse(response: Response, path: string, preview: string, hasToken: boolean): GithubApiError {
  ...
  if (status === 401) {
    return new GithubApiError(
      `GitHub authentication failed (401) for ${path}. The configured GitHub token is missing or invalid; set AMPI_GITHUB_TOKEN (legacy MMR_GITHUB_TOKEN).${detail}`,
      status,
    );
  }
  if (status === 403 && remaining === "0") {
    const advice = hasToken
      ...
      : "The anonymous rate limit is exhausted; set AMPI_GITHUB_TOKEN (legacy MMR_GITHUB_TOKEN) to raise the limit.";
    return new GithubApiError(`GitHub rate limit reached (403) for ${path}. ${advice}${detail}`, status, true);
  }
  if (status === 403) {
    const advice = hasToken ? "" : " This endpoint may require authentication; set AMPI_GITHUB_TOKEN (legacy MMR_GITHUB_TOKEN).";
