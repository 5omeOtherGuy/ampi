 ...
 * enabled. When disabled, no Pi tools are registered; the `ampi-github`
 * provider still emits `gated` decisions so `/ampi-status` explains why.
 */
export function registerMmrGithubTools(
  ...
  for (const factory of TOOL_FACTORIES) {
    ...
    registerAmpiOwnedTool(tool.name);
