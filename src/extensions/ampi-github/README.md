...
Read-only GitHub repository tools for `ampi`. `ampi-github` is also the repository-provider prerequisite for the `ampi-workers` `librarian` worker.
...
| --- | --- | --- | --- |
| Off | GitHub read/list/search/diff tools | `AMPI_GITHUB_ENABLE=true`; token for search/private | `/ampi-status`, tool result `details` |
...
- Inspect private repositories already available to `AMPI_GITHUB_TOKEN`.
- Keep GitHub access read-only and feature-gated.

## Status and enablement

Disabled by default. Set `AMPI_GITHUB_ENABLE=true` (legacy `MMR_GITHUB_ENABLE` still accepted) or `ampiGithub.enabled=true` and restart Pi to register the tools.
...
```json
{
  "ampiGithub": { "enabled": true }
}
```

```bash
export AMPI_GITHUB_ENABLE=true
export AMPI_GITHUB_TOKEN="ghp_xxx"   # optional; env only
```

The `AMPI_GITHUB_*` env names below are canonical; the legacy `MMR_GITHUB_*` equivalents are still accepted.
...
`ampi-github` records its extension entrypoint path so consumers can confirm that a live GitHub tool registration belongs to this extension by source path, not just by name.

`librarian` stays `gated` until every required GitHub tool is registered and source-owned by `ampi-github`. Same-named tools from other extensions do not satisfy the gate.
...
Re-exported from `ampi`: `createAmpiGithubExtension`, settings helpers, feature-gate and tool-provider helpers, ownership predicates, tool-name constants, parser/client errors, and tool registration helpers. Canonical catalog: [`../../../docs/public-api.md`](../../../docs/public-api.md).
