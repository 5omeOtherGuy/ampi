...
import { REVIEWER_BACKGROUND_GUIDANCE } from "../mmr-core/worker-tool-guidance.js";
...
function requireReviewerProfile() {
  ...
  if (!profile) {
    throw new Error(
      `mmr-core does not expose a "${REVIEWER_SUBAGENT_PROFILE}" subagent profile; reviewer cannot run without it.`,
    );
  }
  return profile;
}
 ...
 *
 * Source of truth: the `reviewer` subagent profile in mmr-core. This
 ...
 * `mmr-workers/prompts.ts`, which also registers the reviewer builder
...
function buildProgressDetails(
  ...
  return { worker: "mmr-subagents.reviewer", ...base };
}
...
function buildDetails(
  ...
  return { worker: "mmr-subagents.reviewer", status, ...base };
}
...
export function registerReviewerTool(pi: ExtensionAPI, deps: ReviewerToolDeps = {}): ToolDefinition {
  ...
  registerMmrOwnedTool(REVIEWER_TOOL_NAME);
