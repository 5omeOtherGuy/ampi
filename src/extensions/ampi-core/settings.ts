...
function readNamedSettingsBlock(
  value: Record<string, unknown>,
  flatKey: string,
  nestedRootKey: string,
  nestedKey: string,
  context: { filePath: string; warnings: string[]; label: string },
): { raw?: Record<string, unknown>; rootKey: string } {
  const hasFlat = flatKey in value;
  const flatValue = value[flatKey];
  const flat = isRecord(flatValue) ? flatValue : undefined;
  if (hasFlat && !flat) {
    context.warnings.push(
      `Ignoring ${flatKey} in ${context.filePath}: expected an object of ${context.label} settings.`,
    );
  }

  const nestedRoot = isRecord(value[nestedRootKey]) ? value[nestedRootKey] : undefined;
  const hasNested = nestedRoot ? nestedKey in nestedRoot : false;
  const nestedValue = nestedRoot ? (nestedRoot as Record<string, unknown>)[nestedKey] : undefined;
  const nested = isRecord(nestedValue) ? nestedValue : undefined;
  if (hasNested && !nested) {
    context.warnings.push(
      `Ignoring ${nestedRootKey}.${nestedKey} in ${context.filePath}: expected an object of ${context.label} settings.`,
    );
  }

  return flat ? { raw: flat, rootKey: flatKey } : { raw: nested, rootKey: `${nestedRootKey}.${nestedKey}` };
}

function selectMmrCoreSettingsBlock(
  value: Record<string, unknown>,
  context: { filePath: string; warnings: string[] },
): { raw: Record<string, unknown>; rootKey: string } | undefined {
  const ampi = readNamedSettingsBlock(value, "ampiCore", "ampi", "core", {
    ...context,
    label: "ampi core",
  });
  if (ampi.raw) return { raw: ampi.raw, rootKey: ampi.rootKey };

  const legacy = readNamedSettingsBlock(value, "mmrCore", "mmr", "core", {
    ...context,
    label: "legacy MMR core",
  });
  return legacy.raw ? { raw: legacy.raw, rootKey: legacy.rootKey } : undefined;
}
...
function extractMmrCoreSettings(
  ...
  const selected = selectMmrCoreSettingsBlock(value, context);
