...
import { parseBoolEnv, readPreferredEnv } from "./internal/env.js";
...
const AMPI_CHANGELOG_DEBUG_ENV_VAR = "PI_AMPI_CHANGELOG_DEBUG";
const DEBUG_ENV_VAR = "PI_MMR_CHANGELOG_DEBUG";

function isChangelogDebugEnabled(): boolean {
  return parseBoolEnv(readPreferredEnv(process.env, AMPI_CHANGELOG_DEBUG_ENV_VAR, DEBUG_ENV_VAR)?.value) ?? false;
}
...
export function formatMmrChangelogNotice(display: MmrChangelogDisplay): string {
  ...
  return [
    ...
    "",
    "Use /ampi-changelog to view the full ampi changelog.",
  ].join("\n");
}
