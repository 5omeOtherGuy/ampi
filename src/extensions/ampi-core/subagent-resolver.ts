 ...
 * `ampi-workers` scans for this exact prefix to convert the run into a
 ...
 *  - child activation (`ampi-core/index.ts` `applySubagentProfile`) to
...
> {
   ...
   *    in `ampi-core/index.ts`) is validating its CLI flags. The parent
