 ...
 *
 * `/ampi-status` and the activation notification's "Warnings:" block both render
 * `message` verbatim, so the two surfaces stay in sync. Worker tools and later
 * extensions can branch on the stable `code` field.
 *
 * Deferred-tool messages (e.g. "oracle: deferred until ampi-workers ships")
 ...
 * separate `Deferred tools:` section in `/ampi-status` and as their own bullets
