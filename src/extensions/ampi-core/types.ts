 ...
 * applications and provider-failure fallbacks rendered by `/ampi-status debug`.
 *
 ...
 *   tool yet. Used to give /ampi-status a stable owner for catalog entries.
...
export interface MmrToolProvider {
  /** Provider identifier shown in `/ampi-status` and decision diagnostics; doubles as default owner. */
  name: string;
  /**
 ...
 * their own dynamically-discovered tools (e.g. ampi-workers custom `sa__*`
...
export interface MmrToolDecision {
  ...
  /** Owner extension that produced the rule (e.g. `ampi-core`, `ampi-web`, `ampi-workers`). */
  owner: string;
  /** User-facing diagnostic text surfaced in `/ampi-status` and activation warnings. */
  diagnostic: string;
}
...
export interface MmrFeatureGateProvider {
  /** Provider identifier shown in `/ampi-status` and decision diagnostics. */
  name: string;
...
export interface MmrModeState {
  ...
  /** Runtime-only baseline diagnostics used by /ampi-status; not persisted. */
   ...
   * during the most recent settings load. Surfaced by `/ampi-status` for
   ...
   * MMR-specific blocks). Surfaced by `/ampi-status` so users can see and
   ...
   * `/ampi-status` via the policy-diagnostics pipeline; never persisted and
 ...
 * downstream extensions (ampi-toolbox task_list, ampi-workers, ampi-history)
...
export interface MmrPlannedToolMetadata {
  ...
  name: string;
  /** Planned owner extension identifier (e.g. `ampi-history`, `ampi-workers`). */
  owner: string;
...
/**
 * Stable codes for policy diagnostics surfaced by ampi-core.
 *
 * Worker tools and later extensions can branch on `code` and ignore prose;
 * `message` is the human-readable text that `/ampi-status` and activation
...
export interface MmrPolicyDiagnostic {
  ...
  message: string;
  /** Identifier of the producer; ampi-core diagnostics always use "mmr-core". */
  source: string;
