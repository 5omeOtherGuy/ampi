 ...
 * debugging aid surfaced by `/ampi-status debug`, not an audit log.
 */
export const MMR_MODE_HISTORY_LIMIT = 16;

/**
 * Live runtime snapshot of an active subagent worker. Set only while a
 * `--ampi-subagent <name>` child Pi process is running; cleared when no
 * subagent is active. Distinct from `MmrModeState` so subagent activation
 * never appears as a locked user-facing mode in /ampi-status, the footer,
...
export interface MmrSubagentState {
  /** Subagent profile name (matches `--ampi-subagent <name>`). */
  profile: string;
   ...
   * ampi-core's `before_provider_request` hook in the child Pi process.
...
/**
 * Pi event-bus topic emitted by the ampi-core extension whenever the active
...
function cloneMmrModeState(state: MmrModeState): MmrModeState {
  // Structured-clone-style deep copy. ampi-core's state shape is plain JSON
...
/**
 * Minimal slice of Pi's `EventBus` that ampi-core needs to subscribe to the
...
export interface MmrCoreRuntime {
   ...
   * flow from Pi's session lifecycle in ampi-core, not be poked by arbitrary
   * callers.
   */
 ...
 * `ampi-core` build, the old singleton already stored on `globalThis` may
 ...
 * module-local singleton would give `ampi-core` and sibling extensions
 ...
 * loaded build of `ampi-core` left an older singleton on `globalThis`: if any
...
function resolveProcessRuntime(): MmrCoreRuntime {
  const existing = globalRuntimeStore[AMPI_CORE_RUNTIME_GLOBAL_KEY] ?? globalRuntimeStore[MMR_CORE_RUNTIME_GLOBAL_KEY];
  if (isMmrCoreRuntimeCompatible(existing)) {
    globalRuntimeStore[AMPI_CORE_RUNTIME_GLOBAL_KEY] = existing;
    globalRuntimeStore[MMR_CORE_RUNTIME_GLOBAL_KEY] = existing;
    return existing;
  }
  const fresh = createMmrCoreRuntime();
  globalRuntimeStore[AMPI_CORE_RUNTIME_GLOBAL_KEY] = fresh;
 ...
 * `--ampi-subagent <name>` worker is active. The returned object is the
 ...
 * ampi-core's extension entrypoint (and tests resetting the singleton) can
 ...
 * modes at activation time (e.g. ampi-workers enabled custom `sa__*`
 ...
 * `/ampi-status debug`. Consecutive duplicates are collapsed by the runtime.
 */
