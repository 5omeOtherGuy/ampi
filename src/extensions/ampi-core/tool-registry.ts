...
export const CORE_OWNER = "ampi-core";

/**
 * Exact-name status catalog for tools owned by ampi extensions that may not
 ...
 *    owner (e.g. ampi-web for `web_search`). This keeps /ampi-status
...
const DEFAULT_TOOL_CATALOG: Record<string, { owner: string }> = {
  // ampi-patch
  apply_patch: { owner: "ampi-patch" },
  // ampi-tasks
  task_list: { owner: "ampi-tasks" },
  chart: { owner: "ampi-tasks" },
  // ampi-toolbox-mcp
  read_mcp_resource: { owner: "ampi-toolbox-mcp" },
  // ampi-web
  web_search: { owner: "ampi-web" },
  read_web_page: { owner: "ampi-web" },
  // ampi-workers
  Task: { owner: "ampi-workers" },
  finder: { owner: "ampi-workers" },
  oracle: { owner: "ampi-workers" },
  librarian: { owner: "ampi-workers" },
  reviewer: { owner: "ampi-workers" },
  start_task: { owner: "ampi-workers" },
  task_poll: { owner: "ampi-workers" },
  task_wait: { owner: "ampi-workers" },
  task_cancel: { owner: "ampi-workers" },
  // ampi-history
  find_session: { owner: "ampi-history" },
  read_session: { owner: "ampi-history" },
  handoff: { owner: "ampi-history" },
  // ampi-skills
  skill: { owner: "ampi-skills" },
};
    ...
    resolve(requestedTools, availableTools) {
      ...
      for (const requested of requestedTools) {
        ...
        if (matched) {
          ...
          // the catalog owner when one exists (so /ampi-status credits the
          // shipping extension), else ampi-core.
