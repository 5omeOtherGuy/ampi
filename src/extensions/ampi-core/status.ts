...
function getContextOverridesForState(state: MmrModeState) {
  ...
  // (e.g. openai-codex), explicitly omit the value from display so /ampi-status
...
export function formatFooterTokens(count: number): string {
  ...
  // the lower tiers share ampi-core's compact formatter so the footer and the
...
/**
 * Structural shape of the optional Pi `SessionManager` surfaces ampi-core's
...
export function updateMmrStatus(ctx: ExtensionContext, state: MmrModeState | undefined): void {
  if (!state) {
    ctx.ui.setStatus("ampi-mode", undefined);
