...
/**
 * Context handed to a `/ampi-config` or legacy `/mmr-config` section handler. Kept intentionally small;
...
/**
 * A sibling-owned section of the `/ampi-config` menu. Extensions register their
 * own config sub-flows here instead of `ampi-core` importing them, inverting the
 * former core -> sibling dependency.
 */
export interface MmrConfigFlowSection {
  /** Stable id; re-registration with the same id replaces the prior entry. */
  readonly id: string;
  /** Menu label shown in the `/ampi-config` picker. */
...
const AMPI_CONFIG_FLOW_SECTIONS_GLOBAL_KEY = "__pi_ampi_config_flow_sections_v1__";
const MMR_CONFIG_FLOW_SECTIONS_GLOBAL_KEY = "__pi_mmr_config_flow_sections_v1__";
const globalStore = globalThis as typeof globalThis & {
  [AMPI_CONFIG_FLOW_SECTIONS_GLOBAL_KEY]?: Map<string, MmrConfigFlowSection>;
  [MMR_CONFIG_FLOW_SECTIONS_GLOBAL_KEY]?: Map<string, MmrConfigFlowSection>;
};
const sections: Map<string, MmrConfigFlowSection> = globalStore[AMPI_CONFIG_FLOW_SECTIONS_GLOBAL_KEY]
  ?? globalStore[MMR_CONFIG_FLOW_SECTIONS_GLOBAL_KEY]
  ?? new Map<string, MmrConfigFlowSection>();
globalStore[AMPI_CONFIG_FLOW_SECTIONS_GLOBAL_KEY] = sections;
globalStore[MMR_CONFIG_FLOW_SECTIONS_GLOBAL_KEY] = sections;

/**
 * Register (or replace, by `id`) an `/ampi-config` section. Idempotent: safe to
