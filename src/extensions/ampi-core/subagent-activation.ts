 ...
 * mutable settings/guard state that lives on the ampi-core mode-controller
 ...
 * `ampi-core`'s generic owned-tools registry). Profiles without requirements
 * (the common case) are a no-op. This keeps `ampi-core` free of imports from
 * the owning sibling extensions (e.g. `ampi-github`).
 */
...
export async function applyMmrSubagentProfile(
  ...
  // subagent override block (if any) here so `/ampi-config` writes can
  ...
  const envOverride = parseMmrSubagentModelPreferencesEnv(
    readPreferredEnv(process.env, AMPI_SUBAGENT_MODEL_PREFERENCES_ENV, MMR_SUBAGENT_MODEL_PREFERENCES_ENV)?.value,
  );
