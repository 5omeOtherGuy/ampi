 ...
 *
 * Pure data + a single shape-detecting transformer used by ampi-core's
...
// (and the shared ampi-core/token-format.ts compact tiers). This policy display
 ...
 * `max_output_tokens`. ampi-core uses this both at the wire layer (to skip
 * the field) and at the display layer (to omit it from /ampi-status, footer
 * status, etc.).
 */
 ...
 *
 * ampi-core never enlarges displayed context metadata above what the registered
 ...
 *
 * Display-clamp only: this function shapes `/ampi-status` and footer numbers.
 ...
 * future custom providers continue to work without an ampi-core update.
 */
