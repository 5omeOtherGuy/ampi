 ...
 * functions; ampi-core never injects side-effecting dependencies into them.
 */
...
const AMPI_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY = "__pi_ampi_subagent_prompt_builders_v1__";
...
const globalBuilderStore = globalThis as typeof globalThis & {
  [AMPI_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY]?: Map<string, MmrSubagentPromptBuilder>;
 ...
 * module-local Map would give `ampi-core` (where the registry type lives)
 * and sibling worker extensions (where concrete builders are registered)
 * different registries even though they run in the same Pi process. The
 * singleton is keyed on `globalThis` for the same reason `ampi-core/runtime.ts`
 * keeps its registry there.
 */
function resolveBuilderRegistry(): Map<string, MmrSubagentPromptBuilder> {
  const existing = globalBuilderStore[AMPI_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY]
    ?? globalBuilderStore[MMR_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY];
  if (existing instanceof Map) {
    globalBuilderStore[AMPI_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY] = existing;
    globalBuilderStore[MMR_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY] = existing;
    return existing;
  }
  const fresh = new Map<string, MmrSubagentPromptBuilder>();
  globalBuilderStore[AMPI_SUBAGENT_PROMPT_BUILDERS_GLOBAL_KEY] = fresh;
 ...
 * because concrete prompt-owner extensions are
 * authoritative.
 */
...
function resolveModeDerivedBaseMode(profile: MmrSubagentProfile, parentMode: MmrModeKey | undefined): MmrModeKey {
  if (profile.baseMode === "from-parent") {
    if (parentMode === undefined) {
      throw new Error(
        `ampi-core: subagent profile "${profile.name}" declares baseMode "from-parent" but assembleMmrSubagentSurface was called without parentMode`,
      );
    }
    return parentMode;
  }
  if (profile.baseMode === undefined) {
    throw new Error(`ampi-core: mode-derived subagent profile "${profile.name}" does not declare baseMode`);
  }
...
function buildMinimalBaseModeState(baseMode: MmrModeKey): MmrModeState {
  ...
  // ampi-core does not consult model/tool fields here; this state is
...
export function assembleMmrSubagentSurface(
  ...
  if (!builder) {
    throw new Error(
      `ampi-core: no subagent prompt builder registered for "${profile.promptBuilder}" (profile "${profile.name}")`,
    );
  }
  ...
  if (modeState !== undefined && modeState.mode !== resolvedBaseMode) {
    throw new Error(
      `ampi-core: subagent profile "${profile.name}" resolved baseMode "${resolvedBaseMode}" but modeState.mode is "${modeState.mode}"`,
    );
  }
  ...
  if (activeToolsBlock) {
    ...
    // ampi-core rebuilt it from the subagent-filtered worker manifest so
