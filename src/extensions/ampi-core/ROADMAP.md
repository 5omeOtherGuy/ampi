...
This roadmap covers the `ampi-core` extension: locked modes, model
...
`ampi-core` provides the locked-mode spine:
...
- logical ampi tool-name resolution to registered Pi tools
- active tool filtering and `tool_call` blocking for locked ampi modes
- fail-closed zero-active-tools policy for locked mode activation
- `free` mode for native Pi model/thinking controls, baseline Pi tools, and no ampi prompt/tool enforcement
...
- persisted `ampi-core.mode-state` session entries
...
- shared helpers for later ampi modules

## Guiding boundaries

`ampi-core` owns locked-mode consistency only. It should not implement:

- `Task`, `finder`, `oracle`, or `librarian` (belong to `ampi-workers`)
- handoff (belongs to `ampi-history`)
- toolbox protocol or MCP bridge (belong to `ampi-toolbox-mcp`)
- broader provider parity beyond the narrow per-mode token/reasoning
  rewrites already shipped (belongs to `ampi-provider-parity`)

Those belong in later dedicated ampi extensions.
...
Goal: make ampi mode state a stable contract, not just a static table.
...
- ✅ `ampi-core.mode-state` entries persist explicit mode changes and restore the latest valid mode on session start.
- ✅ Feature-gate resolver with an ordered provider chain. Built-ins: `ampi-core.reserved` and `ampi-core.unknown`. For shipped extensions (`ampi-workers`, `ampi-web`) the reserved entry is a fallback only — those extensions ship their own feature-gate providers that take precedence. `MmrFeatureGateProvider` is the documented extension point for future server or package-provided gates; `registerMmrFeatureGateProvider(...)` plugs them into the runtime registry, and later registrations override earlier ones.
...
- ✅ `ampi-core.mode-state` entries are versioned (`MMR_MODE_STATE_VERSION = 1`); legacy unversioned records are normalized to v1, future/malformed versions are rejected.

Acceptance criteria:

- ✅ `getMmrModeState()` returns a complete, versioned, explainable locked-mode state.
- ✅ `/ampi-status` can explain why the current mode/model/tools were chosen, including per-gate provider attribution.
...
- ✅ Prompt assembly remains ampi-named and does not expose historical source naming as public UX.

## Milestone 4 — shared contracts for later modules

Status: ✅ Complete.

Goal: let future extensions plug into `ampi-core` without duplicating locked-mode state.
...
- ✅ Public `ampi-core` API documented in [`../../../docs/ampi-core-api.md`](../../../docs/ampi-core-api.md).
...
- ✅ A future module can register a tool provider and have `ampi-core` include it on the next mode resolution. (`registerMmrToolProvider`.)
...
- ✅ `/ampi-status` reports active locked mode/source, resolved model/applied/configured fallback, thinking, prompt surface, active tools, missing logical tools, deferred tools, gated tools, settings files read, and diagnostics grouped by severity.
...
- ✅ Optional `Debug` section in `/ampi-status` for model/tool resolution diagnostics.
...
- [ ] Add a `docs/troubleshooting.md` covering common failure modes:
  ...
  - `ampi-web` disabled or `web_search` missing `BRAVE_API_KEY`

Acceptance criteria:

- ✅ Users can diagnose common setup problems from `/ampi-status` output.
...
- ✅ Pi-authored auto-section blocks (`Available tools:`, `Guidelines:`, `Pi documentation`) are passed through byte-identically; no ampi-side filtering of Pi bullets.
...
- ✅ Pi-authored prompt blocks remain byte-identical through ampi assembly.
...
`ampi-workers` (and future packages) can apply a profile-resolved
model / thinking level / tool allowlist verbatim without inheriting
locked-mode policy or accidentally interacting with it.

Implemented:

- ✅ `--ampi-subagent <name>` CLI flag registered in the child Pi process.
...
- ✅ `applySubagentProfile` activates the resolved route via
  ...
  `--ampi-subagent` is absent, `session_start` explicitly clears any
...
- ✅ Subagent activation does NOT capture/restore a Pi baseline, emit
  `MMR_EVENT_STATE_CHANGED`, persist `ampi-core.mode-state`, apply
...
- ✅ Pure stderr parser (`extractMmrSubagentActivationFailure`)
  consumed by the `ampi-workers` runner to convert the marker into
...
`ampi-core` updates subagent activation state.

Acceptance criteria:

- ✅ Subagent activation never captures/restores a Pi baseline, never
  emits `MMR_EVENT_STATE_CHANGED`, never persists `ampi-core.mode-state`,
  never applies locked-mode prompt templates, never injects ampi-owned
...
- ✅ Subagent activation fails closed on unknown profile, no model
  ...
  and live verified through real `pi --ampi-subagent` invocations.)
...
- ✅ Failure marker is a stable shared constant between producer
  (ampi-core) and consumer (runner); drift is impossible. (Pinned by
...
- Wrappers should only be introduced when a specific, named behavioral gap
  cannot be closed by prompt-side steering or by `ampi-toolbox` / `ampi-web` /
