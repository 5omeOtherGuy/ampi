 ...
 * `ampi-core` from `--ampi-subagent <name>` in the child Pi process and
 ...
 * validates these fail-closed via `ampi-core`'s generic owned-tools registry,
 * which lets `ampi-core` gate a worker (e.g. `librarian` on `ampi-github` repo
 ...
 *
 * Consumed by the shared worker-outcome classifier in `ampi-workers`;
 * `ampi-core` only declares the bit so every surface (blocking tools,
...
export interface MmrSubagentProfile {
  /** Profile identifier used by `--ampi-subagent`. */
   ...
   * the child Pi process applies it through ampi-core's
   ...
   * declared extension. Lets `ampi-core` gate owner-specific tools (e.g.
   * `librarian` repo tools owned by `ampi-github`) without importing the owner.
   */
  ...
  /**
   * Whether the worker may surface `ampi-toolbox` mutation tools
...
/**
 * Read-only GitHub repository provider tools owned by `ampi-github`. The
 * `librarian` profile activates these by name and also requires them to be
 * `ampi-github`-owned at runtime; defined once so the allowlist and the
...
const MMR_SUBAGENT_PROFILE_TABLE: Record<string, MmrSubagentProfile> = {
  finder: deepFreeze({
    ...
    // Providers without such a lane resolve `low` via ampi-core's
  ...
  "history-reader": deepFreeze({
    ...
    tools: [],
    maxTurns: 1,
    // Internal extraction worker driven by the ampi-history read tools; it is
  ...
  oracle: deepFreeze({
    ...
    // ampi-history) are listed here for honest profile intent and are
  ...
  librarian: deepFreeze({
    ...
    // Fail closed unless the repo tools are present AND owned by `ampi-github`,
    // validated through ampi-core's generic owned-tools registry (no import of
    // ampi-github from core).
    requiredOwnedTools: [
      {
        owner: "ampi-github",
        toolNames: [...LIBRARIAN_REPO_TOOLS],
        description: "ampi-github-owned read-only GitHub tools",
        unmetHint: "set AMPI_GITHUB_ENABLE=true or legacy MMR_GITHUB_ENABLE=true",
      },
    ],
    promptRoute: "standalone",
...
for (const profile of Object.values(MMR_SUBAGENT_PROFILE_TABLE)) {
  if (profile.promptRoute === "standalone" && profile.baseMode !== undefined) {
    throw new Error(
      `ampi-core subagent profile "${profile.name}" is standalone but declares baseMode "${profile.baseMode}"`,
    );
  }
  if (profile.promptRoute === "mode-derived" && profile.baseMode === undefined) {
    throw new Error(
      `ampi-core subagent profile "${profile.name}" is mode-derived but does not declare a baseMode`,
    );
  }
}
 ...
 */
const AMPI_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY = "__pi_ampi_dynamic_subagent_profiles_v1__";
...
const globalProfileStore = globalThis as typeof globalThis & {
  [AMPI_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY]?: Map<string, MmrSubagentProfile>;
...
function resolveDynamicProfileRegistry(): Map<string, MmrSubagentProfile> {
  const existing = globalProfileStore[AMPI_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY]
    ?? globalProfileStore[MMR_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY];
  if (existing instanceof Map) {
    globalProfileStore[AMPI_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY] = existing;
    globalProfileStore[MMR_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY] = existing;
    return existing;
  }
  const fresh = new Map<string, MmrSubagentProfile>();
  globalProfileStore[AMPI_DYNAMIC_SUBAGENT_PROFILES_GLOBAL_KEY] = fresh;
...
/**
 * Register or replace a runtime subagent profile. Used by ampi-workers for
 * user-authored Markdown subagents whose profile shape is discovered at
 * extension activation instead of being shipped in ampi-core's static table.
 */
