...
/**
 * Core-owned pure result shaping for spawned-subagent tools (moved from
 * `ampi-workers/worker-result-shaping.ts`, which re-exports this module) so
 * sibling extensions can build worker details through the worker-host seam
 * without importing `ampi-workers`. Concentrates the small, behavior-preserving
 ...
 *    (`"ampi-workers.finder"`, `"ampi-workers.oracle"`, etc.);
