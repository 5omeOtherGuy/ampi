...
import { getMmrMode, isMmrModeKey } from "../ampi-core/modes.js";
import {
  ...
  setMmrModeState,
} from "../ampi-core/runtime.js";
import { updateMmrStatus } from "../ampi-core/status.js";
...
function clearSessionFallback(pi: ExtensionAPI, ctx: ExtensionContext, reason: string): void {
  ...
  try {
    pi.appendEntry(AMPI_SESSION_FALLBACK_ENTRY, toPersistedMmrSessionFallbackClear({
      sessionId,
      reason,
...
async function handleMessageEnd(pi: ExtensionAPI, event: { message?: unknown }, ctx: ExtensionContext): Promise<{ message: MmrMessageEndReplacement } | undefined> {
  ...
  try {
    pi.appendEntry(AMPI_SESSION_FALLBACK_ENTRY, override);
  } catch (error) {
