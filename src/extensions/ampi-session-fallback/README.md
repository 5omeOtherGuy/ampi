...
| --- | --- | --- | --- |
| On | Interactive quota/capacity fallback prompt + session-scoped override | none | `/ampi-status` (`Configured fallback:`), session-log custom entries |
...
Always loaded. No-op unless **all** of: locked ampi mode active (not `free`), no subagent worker running, no fallback prompt already in flight, and no override already applied for the current session. Otherwise Pi's native error handling runs unchanged.
...
- `pi.setModel(...)` and `pi.setThinkingLevel(...)` are called inside `runMmrManagedModelUpdate(...)` so `ampi-core`'s native-control Free-mode opt-out is not triggered.
- `setMmrManagedModelOverride({ kind: "session-fallback", ... })` records the override on the `ampi-core` runtime.
- `ampi-core`'s mode-state snapshot is republished with `modelFallbackApplied: true`, `modelFallbackReason`, new `provider`/`model`/`thinkingLevel`, and refreshed `effectiveContextWindow`, so `/ampi-status` reflects the fallback.
- A `ampi-session-fallback.override` custom session entry is appended ([`state.ts`](state.ts)) keyed to `sessionId`: `{ version, sessionId, mode, failingProvider, failingModel, selectedProvider, selectedModel, thinkingLevel, reasonKind, appliedAt }`. A `cleared: true` variant records explicit clears.
...
| Event                                          | Behavior                                                                                                                                                                                                              |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `session_start` (`new` / `fork`)               | Clear in-process override and any `ampi-core` managed-model override.                                                                                                                                                  |
...
- **Fallback did not trigger.** Error was not classified as quota/rate-limit/capacity, the run is inside a subagent worker, the mode is `free`, or a prompt/override is already active for this session. Check `/ampi-status` for `Configured fallback:` and inspect the original error against the classifier table above.
...
Re-exported from `ampi`. The `ampi/extensions/ampi-session-fallback` subpath exposes the extension factory entrypoint only.

- `createMmrSessionFallbackExtension()` — Pi extension factory.
- `classifyMmrSessionFallbackError(input)` — pure classifier.
- `AMPI_SESSION_FALLBACK_ENTRY`, `MMR_SESSION_FALLBACK_STATE_VERSION` — persisted-entry constants.
