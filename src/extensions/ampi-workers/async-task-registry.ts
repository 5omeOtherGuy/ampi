...
const AMPI_ASYNC_TASK_REGISTRY_GLOBAL_KEY = "__pi_ampi_workers_async_task_registry_v1__";
...
const globalRegistryStore = globalThis as typeof globalThis & {
  [AMPI_ASYNC_TASK_REGISTRY_GLOBAL_KEY]?: MmrAsyncTaskRegistry;
 ...
 * `ampi-core/runtime.ts`: Pi may load extension entrypoints with isolated
...
export function getMmrAsyncTaskRegistry(): MmrAsyncTaskRegistry {
  const existing = globalRegistryStore[AMPI_ASYNC_TASK_REGISTRY_GLOBAL_KEY]
    ?? globalRegistryStore[MMR_ASYNC_TASK_REGISTRY_GLOBAL_KEY];
  if (isRegistryCompatible(existing)) {
    globalRegistryStore[AMPI_ASYNC_TASK_REGISTRY_GLOBAL_KEY] = existing;
    globalRegistryStore[MMR_ASYNC_TASK_REGISTRY_GLOBAL_KEY] = existing;
    return existing;
  }
  const fresh = createMmrAsyncTaskRegistry();
  globalRegistryStore[AMPI_ASYNC_TASK_REGISTRY_GLOBAL_KEY] = fresh;
