...
export interface MmrWorkersFactoryOverrides {
  ...
  reviewer?: ReviewerToolDeps;
  asyncTasks?: AsyncTaskToolDeps;
}

/**
 * Build a Pi extension factory for `ampi-workers` with optional test seams.
 *
 ...
 *
 * ampi-workers is the merged worker extension: the blocking `finder`,
 * `oracle`, `Task`, and `librarian` workers plus the background task
 * surface (`start_task`, `task_poll`, `task_wait`, `task_cancel`) ship
 * together behind one feature gate (`ampi-workers`; the pre-merge gate ids
...
export function createMmrWorkersExtension(overrides: MmrWorkersFactoryOverrides = {}) {
  return function ampiWorkersExtension(pi: ExtensionAPI): void {
    // Register concrete subagent prompt builders against ampi-core's
    ...
    registerReviewerTool(pi, overrides.reviewer ?? {});
    ...
    const asyncPushCeiling = parseBoolEnv(
      readPreferredEnv(process.env, AMPI_SUBAGENTS_ASYNC_PUSH_ENV, MMR_SUBAGENTS_ASYNC_PUSH_ENV)?.value,
    ) ?? true;
...
export const createAmpiWorkersExtension = createMmrWorkersExtension;

const ampiWorkersExtension = createMmrWorkersExtension();

export default ampiWorkersExtension;
