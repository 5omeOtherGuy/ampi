 ...
 * advisory workers backed by an ampi-core standalone profile.
 */
export function requireMmrAdvisorProfile(profileName: string): MmrSubagentProfile {
  ...
  if (!profile) {
    throw new Error(
      `ampi-core does not expose a "${profileName}" subagent profile; the advisor cannot run without it.`,
    );
  }
  return profile;
}
 ...
 * `ampi-core`. Tools whose owning extension is not loaded in the child
 * Pi process (e.g. `read_session` / `find_session` when `ampi-history`
 ...
 * profile in `ampi-core` (the single source of truth).
 *
 ...
 * `## Using workers` block (`ampi-core/worker-tool-guidance.ts`). The `files`
...
export interface MmrAdvisorToolConfig {
  ...
  toolName: string;
  /** ampi-core subagent profile name driving model/tools/prompt. */
  profileName: string;
  ...
  return createWorkerTool<OracleParams, OracleDetails, InternalAttachment[]>(
    {
      ...
      // `resolveMmrSubagentInvocation` in ampi-core). Parent must not pass
      // explicit --tools here because the profile lists tools whose
      // owning extension may be unloaded in the child environment (e.g.
      // ampi-web's web_search / read_web_page, ampi-history's read_session
...
export const ORACLE_TOOL_CONFIG: MmrAdvisorToolConfig = {
  ...
  workerDiscriminator: "ampi-workers.oracle",
 ...
 * it as ampi-owned so Free mode strips it like every other ampi-authored
 * tool.
 */
export function registerOracleTool(pi: ExtensionAPI, deps: OracleToolDeps = {}): ToolDefinition {
  ...
  registerAmpiOwnedTool(ORACLE_TOOL_NAME);
