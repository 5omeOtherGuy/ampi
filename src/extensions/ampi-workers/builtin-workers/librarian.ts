...
function requireLibrarianProfile() {
  ...
  if (!profile) {
    throw new Error(
      `ampi-core does not expose a "${LIBRARIAN_SUBAGENT_PROFILE_NAME}" subagent profile; librarian cannot run without it.`,
    );
  }
  return profile;
}
 ...
 * (`ampi-core/worker-tool-guidance.ts`).
 */
...
export interface LibrarianToolDeps {
   ...
   * parent and child activation paths cannot drift after `/ampi-config`.
   */
  modelPreferencesOverride?: readonly MmrModelPreference[];
...
export function registerLibrarianTool(pi: ExtensionAPI, deps: LibrarianToolDeps = {}): ToolDefinition {
  ...
  registerAmpiOwnedTool(LIBRARIAN_TOOL_NAME);
