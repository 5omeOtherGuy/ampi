...
function requireTaskProfile() {
  ...
  if (!profile) {
    throw new Error(
      `ampi-core does not expose a "${TASK_SUBAGENT_PROFILE}" subagent profile; Task cannot run without it.`,
    );
  }
  return profile;
}
...
export interface TaskToolDeps {
   ...
   * `/ampi-config` change takes effect on the next invocation, matching
...
export function registerTaskTool(pi: ExtensionAPI, deps: TaskToolDeps = {}): ToolDefinition {
  ...
  registerAmpiOwnedTool(TASK_TOOL_NAME);
