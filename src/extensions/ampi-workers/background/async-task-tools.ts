...
export interface AsyncTaskToolDeps extends TaskToolDeps {
  ...
  /** Tool-specific seams used when start_task launches the reviewer agent. */
  reviewerDeps?: ReviewerToolDeps;
  /**
...
async function executeBackgroundStart(options: BackgroundStartOptions): Promise<AgentToolResult<AsyncTaskToolDetails>> {
  ...
  if (!started.ok) {
    ...
    return {
      ...
      details: { worker: "ampi-workers.async-task", tool: resultTool, agent: member.agent, errorMessage: message },
    };
  }
...
export function createStartTaskTool(deps: AsyncTaskToolDeps = {}): ToolDefinition {
  ...
  const executeFleet = async (
    ...
    if (runningCount + plan.totalMembers > cap) {
      ...
      return {
        content: [{ type: "text", text: message }],
        details: { worker: "ampi-workers.async-task", tool: START_TASK_TOOL_NAME, errorMessage: message },
      };
    }
    ...
    async execute(_toolCallId, rawParams, _signal, _onUpdate, ctx): Promise<AgentToolResult<AsyncTaskToolDetails>> {
      ...
      if (!control.taskId) {
        ...
        return {
          ...
          details: { worker: "ampi-workers.async-task", tool: TASK_POLL_TOOL_NAME, board },
        };
      }
...
export function registerAsyncTaskTools(pi: ExtensionAPI, deps: AsyncTaskToolDeps = {}): ToolDefinition[] {
  ...
  for (const definition of definitions) {
    registerAmpiOwnedTool(definition.name);
  ...
  if (typeof pi.registerMessageRenderer === "function") {
    ...
    pi.registerMessageRenderer(LEGACY_ASYNC_TASK_COMPLETION_CUSTOM_TYPE, renderAsyncTaskCompletionMessage);
  }
