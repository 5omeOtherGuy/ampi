 ...
 *
 *   1. ampi-core (always) — owns the `--ampi-subagent` activation guard, the tool
 ...
 *  - finder       — tools read/grep/find are Pi built-ins; only ampi-core needed.
 *  - oracle       — read/grep/find (built-in) + web_search/read_web_page
 *                   (ampi-web) + read_session/find_session (ampi-history). The
 *                   nested model-backed `history-reader` assembly is also
 *                   owned by ampi-history.
 *  - librarian    — GitHub repository tools owned by ampi-github.
 *  - task-subagent— read/bash/edit/write/skill (built-in) + read_web_page/
 *                   web_search (ampi-web) + finder (ampi-workers) + task_list
 *                   (ampi-tasks).
 */
export const MMR_SUBAGENT_CHILD_KEEP_EXTENSIONS: Readonly<Record<string, readonly string[]>> = {
  finder: ["ampi-core"],
  oracle: ["ampi-core", "ampi-web", "ampi-history"],
  librarian: ["ampi-core", "ampi-github"],
  "task-subagent": ["ampi-core", "ampi-web", "ampi-workers", "ampi-tasks"],
};
...
/**
 * Detect whether debug request capture is active. ampi-debug registers no
 ...
 * (ampi-core first), followed by all non-ampi loaded paths in first-seen
...
export function resolveMmrChildExtensionScope(
  ...
  const ampiDirName = (candidate: string): string | undefined => {
  ...
  // Canonical ampi keep paths in declared order. ampi-core resolving is the
  ...
  const ampiKeepPaths: string[] = [];
  for (const name of keepNames) {
    const candidate = path.join(extensionsDir, name, `index${moduleExt}`);
    if (fileExists(candidate)) ampiKeepPaths.push(candidate);
  }
  if (ampiKeepPaths.length !== keepNames.length) return undefined;
  ...
  for (const candidate of input.loadedPaths) {
    if (ampiDirName(candidate) !== undefined) continue;
  ...
  const result: string[] = [];
  const seen = new Set<string>();
  for (const candidate of [...ampiKeepPaths, ...externalPaths]) {
 ...
 * lives at `<pkg>/src/extensions/ampi-subagents/child-extension-scope.<ext>`, so
