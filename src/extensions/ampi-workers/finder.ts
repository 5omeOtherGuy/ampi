...
function requireFinderProfile() {
  ...
  if (!profile) {
    throw new Error(
      `ampi-core does not expose a "${FINDER_SUBAGENT_PROFILE}" subagent profile; finder cannot run without it.`,
    );
  }
  return profile;
}
 ...
 * reach the network.
 *
 * Source of truth: the `finder` subagent profile in ampi-core. This export
 ...
 *
 * Source of truth: the `finder` subagent profile in ampi-core. The profile
...
function buildProgressDetails(
  ...
  return { worker: "ampi-workers.finder", ...base };
}
...
function buildDetails(
  ...
  return { worker: "ampi-workers.finder", status, ...base };
}
 ...
 * ampi-owned so Free mode strips it like every other ampi-authored tool.
 */
export function registerFinderTool(pi: ExtensionAPI, deps: FinderToolDeps = {}): ToolDefinition {
  ...
  registerAmpiOwnedTool(FINDER_TOOL_NAME);
