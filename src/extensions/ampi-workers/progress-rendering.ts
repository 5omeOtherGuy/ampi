...
export const ASYNC_TASK_COMPLETION_CUSTOM_TYPE = "ampi-workers.async-task-completion" as const;
export const LEGACY_ASYNC_TASK_COMPLETION_CUSTOM_TYPE = "mmr-subagents.async-task-completion" as const;
...
export interface AsyncTaskCompletionDetails {
  version: 1;
  kind: typeof ASYNC_TASK_COMPLETION_CUSTOM_TYPE | typeof LEGACY_ASYNC_TASK_COMPLETION_CUSTOM_TYPE;
  taskId?: string;
  groupId?: string;
  status: string;
  description?: string;
...
/**
 * Renderer for the `ampi-workers.async-task-completion` push message
 * (and the legacy `mmr-subagents.async-task-completion` custom type).
 *
 ...
 * ampi-workers so it never collides with the ampi-toolbox task-list widget.
 */
 ...
 * Mirrors the ampi-toolbox task-list widget gate so behavior is identical
