...
export interface SanitizedHistoryReaderSessionPacket {
   ...
   * redaction is opted in (AMPI_HISTORY_REDACT, legacy MMR_HISTORY_REDACT);
   * by default content is raw.
   */
  scope: "all_sessions";
...
export interface HistoryReaderWorkerDetails extends MmrSubagentWorkerDetailsBase {
  worker: "ampi-history.history-reader";
...
function requireHistoryReaderProfile() {
  ...
  if (!profile) {
    throw new Error(`ampi-core does not expose a "${HISTORY_READER_SUBAGENT_PROFILE}" subagent profile.`);
  }
  return profile;
}
