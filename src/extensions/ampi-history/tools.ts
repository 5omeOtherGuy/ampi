...
export const FIND_SESSION_PARAMETERS_SCHEMA = Type.Object(
  {
    ...
    limit: Type.Optional(
      Type.Number({
        description: "Maximum number of sessions to return for this page. Defaults to the ampi-history configured limit and is capped by the extension.",
      }),
    ),
...
export function registerMmrHistoryTools(pi: ExtensionAPI, deps: MmrHistoryToolDeps): void {
  const settings = deps.getSettings();
  for (const name of [FIND_SESSION_TOOL_NAME, READ_SESSION_TOOL_NAME]) registerAmpiOwnedTool(name);
