...
| --- | --- | --- | --- |
| Off | `find_session`, `read_session` | `AMPI_HISTORY_ENABLE=true` | `/ampi-status`, tool result `details` |
...
Set `AMPI_HISTORY_ENABLE=true` (legacy `MMR_HISTORY_ENABLE` still accepted) to register both tools. The `AMPI_HISTORY_*` env names below are canonical; the legacy `MMR_HISTORY_*` equivalents are still accepted. Optional caps:

- `AMPI_HISTORY_MAX_RESULTS` — default 10, capped at 20.
- `AMPI_HISTORY_MAX_EXCERPT_BYTES` — default 24,000, capped at 100,000.
...
`read_session` always tries the `history-reader` subagent first. It runs in-process with `tools: []` and receives a sanitized session packet. Model selection: per-call `model` → `ampiCore.subagentModelPreferences["history-reader"]` → profile defaults (`antigravity/gemini-3.5-flash-extra-low` → `gpt-5.4-mini` → `claude-haiku-4-5`, minimal thinking). Packet entry-type allowlist: `message`, `compaction`, `branch_summary`, `session_info`; `custom`, `custom_message`, and `extension` entries are dropped.
...
There is no per-project consent prompt. Content redaction is off by default for local same-user recovery; set `AMPI_HISTORY_REDACT=true` when packet/result content should be sanitized before leaving the local catalog.

## Diagnostics and troubleshooting

- **Tools are `missing` in `/ampi-status`.** `AMPI_HISTORY_ENABLE` is unset; nothing is registered.
...
Re-exported from `ampi`: `createAmpiHistoryExtension`, `MMR_HISTORY_TOOL_NAMES`, history tool/details types. Canonical catalog: [`../../../docs/public-api.md`](../../../docs/public-api.md).
