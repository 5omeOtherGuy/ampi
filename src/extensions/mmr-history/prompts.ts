 ...
 *
 * Owned by `mmr-history`: concrete prompt text lives with the session-history
 * tool that invokes the internal `history-reader` subagent, not in the generic
 * `mmr-subagents` worker bundle.
 */
...
/**
 * Register the `history-reader` prompt builder against mmr-core's
