...
export interface MmrHistorySettings {
   ...
   * through unless the operator explicitly sets `MMR_HISTORY_REDACT` to a
   * truthy value.
   *
...
export function loadMmrHistorySettings(env: NodeJS.ProcessEnv = process.env): MmrHistorySettings {
  return {
    ...
    redactionEnabled: parseBoolEnv(env[MMR_HISTORY_REDACT_ENV]) ?? false,
    packetByteBudget: parsePositiveInteger(
      env[MMR_HISTORY_PACKET_BYTE_BUDGET_ENV],
