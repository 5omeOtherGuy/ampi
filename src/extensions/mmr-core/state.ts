...
// propagate invalid values into runtime state and /mmr-status rendering.
...
export function createMmrModeState(args: {
  ...
  /**
   * Pre-resolved feature-gate decisions. When omitted, mmr-core falls back to
  ...
  /** Optional settings-load metadata; rendered by `/mmr-status` only. */
  settingsFilesRead?: readonly string[];
  /** Optional settings-load warnings; rendered by `/mmr-status` only. */
...
export function findLatestPersistedModeState(entries: readonly unknown[]): PersistedMmrModeState | undefined {
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    ...
    if (entry.type !== "custom" || entry.customType !== MMR_MODE_STATE_ENTRY) continue;
