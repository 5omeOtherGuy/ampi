...
export function resolveMmrModeSelection(input: ResolveMmrModeSelectionInput): MmrModeSelection {
  ...
  const sources: SourceEntry[] = [
    { source: "flag", rejectedName: "flag", invalidDescription: "--mmr-mode", value: input.flagValue },
  ...
  for (const entry of sources) {
    ...
    warnings.push(`Ignoring invalid ${entry.invalidDescription} MMR mode "${candidate}".`);
