...
- **Thinking-level toggle (`alt+r`)**: `smart`, `smartGPT`, and `deep` are toggleable. The MMR-owned `alt+r` shortcut flips the active mode between two presets in place without releasing the mode — `smart` Opus medium↔high, `smartGPT`/`deep` GPT medium↔xhigh. The toggle drives the Pi thinking level and the wire reasoning effort. For `smart`, the Anthropic adaptive effort follows the native Opus route's Pi-level map (Option 1): the medium preset (Pi `medium`) maps to Anthropic `high`, and the high preset (Pi `high`) maps to Anthropic `xhigh`; both presets keep the Anthropic output budget at 32k so the displayed max-input remains 268k on the capped 300k Opus route. OpenAI Responses effort tracks the Pi level (`medium`/`high`). The toggle is lightweight (concise status line, no mode-activation banner) and no-ops in non-toggleable modes. Toggle overrides are session-scoped (process memory); the default preset is re-derived on each apply. A dedicated key is used because Pi reserves `shift+tab` (`app.thinking.cycle`) and extensions cannot override it.
...
- **Context profiles**: `smart` 300k/268k/32k (Anthropic branch), `smartGPT` 256k/128k/128k, `rush` 256k/128k/128k, `large` 1M/968k/32k, `deep` 256k/128k/128k. Every locked mode caps its active model's `contextWindow` down to its advertised profile total via a shallow clone at the `setModel` call site (`context-cap.ts`), derived from the mode's own request policy so the enforced and advertised windows stay in sync. This makes Pi's native compaction, overflow, footer, percent, and `getContextUsage()` run at the profile window even when the route's native window is larger (e.g. a 1M Opus route, or the `openai-codex` `gpt-5.5` route whose registry window of 300k is above Codex's real ~272k input ceiling). The GPT modes' 256k profile keeps Pi's compaction trigger (~240k) and the compaction summary request under Codex's real limit, instead of overflowing with `context_length_exceeded`. Capping is cap-down only, so a smaller custom route stays authoritative, and `free` (no policy) is never capped. The cap is reasserted defensively if a provider (re)registration (e.g. `/login`) transiently re-resolves the active model to its uncapped window.
