...
- **Context profiles**: `smart` 300k/268k/32k (Anthropic branch), `smartGPT` 400k/272k/128k, `rush` 400k/272k/128k, `large` 1M/968k/32k, `deep` 400k/272k/128k. Most locked modes do not clone or rewrite model context windows; smaller routes clamp down for display. `smart` is the explicit exception: it caps the active model's `contextWindow` to 300k via a shallow clone at the `setModel` call site (`context-cap.ts`), so Pi's native compaction, overflow, footer, percent, and `getContextUsage()` all run at 300k even on the 1M Opus route. The cap is reasserted defensively if a provider (re)registration (e.g. `/login`) transiently re-resolves the active model to its uncapped window.
