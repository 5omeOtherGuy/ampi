...
`smart`, `fable`, `rush`, `deep` apply a locked-mode profile (model preferences, request policy, context profile, active-tool allowlist, MMR-owned prompt block). `free` releases all enforcement and restores the pre-MMR baseline.
