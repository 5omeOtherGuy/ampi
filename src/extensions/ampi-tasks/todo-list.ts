...
// mirror `src/extensions/ampi-core/state.ts` so the two persistence layers
...
export const TODO_STATE_ENTRY = "ampi-tasks.todo-state";
/** Legacy persisted entry key read for existing sessions. */
export const LEGACY_TODO_STATE_ENTRY = "mmr-tasks.todo-state";
export const TODO_STATE_VERSION = 2;
const TODO_STATE_ENTRY_KEYS: ReadonlySet<string> = new Set([TODO_STATE_ENTRY, LEGACY_TODO_STATE_ENTRY]);
...
export function findLatestPersistedTodoState(
  ...
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    ...
    if (entry.type !== "custom" || !TODO_STATE_ENTRY_KEYS.has(entry.customType ?? "")) {
      continue;
    }
