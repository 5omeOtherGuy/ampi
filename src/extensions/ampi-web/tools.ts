...
export const READ_WEB_PAGE_PROMPT_SNIPPET =
  "Fetch a public http(s) page through ampi-web's custom reader and return Markdown text";
...
/**
 * Register the concrete Pi tools for `ampi-web` based on the current settings.
 *
 * - When network access is disabled, no Pi tools are registered. The
 *   `ampi-web` tool provider still emits `gated` decisions so users see why
...
export function registerMmrWebTools(pi: ExtensionAPI, deps: MmrWebToolDeps): RegisterMmrWebToolsResult {
  ...
  if (searchDecision.backend) {
    registerAmpiOwnedTool("web_search");
  ...
  if (readerDecision.backend) {
    registerAmpiOwnedTool("read_web_page");
