/**
 * Managed SearXNG sidecar for `ampi-web` (opt-in).
 *
 * When the user opts in by setting `mmrWeb.searxngManaged=true` AND
 * `mmrWeb.searxngStartCommand` in their settings file, `ampi-web`
 ...
 * "set (N args)" arg-count style used by the `ampi-web` config view, so error
