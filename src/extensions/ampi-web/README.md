...
Disabled by default. All locked modes report `web_search` and `read_web_page` as `gated` until you opt in with `AMPI_WEB_ENABLE=true` (legacy `MMR_WEB_ENABLE` still accepted) or `ampiWeb.enabled=true`. Once enabled, `web_search` is always **active** — with no SearXNG URL and no Brave key, it falls back to a best-effort no-key DuckDuckGo HTML scraper so the tool works with zero configuration.

`ampi-web` registers an ampi tool provider and a feature-gate provider; `ampi-core`'s runtime singleton on `globalThis` keeps these visible across Pi loader caches. Tool details report the concrete path used: `WebSearchDetails.backend` is `searxng` / `brave` / `duckduckgo`; `ReadWebPageDetails.backend` is `custom`.
...
Schemas: `web_search` requires `objective`; optional `search_queries`, `max_results`, `include_domains`, `exclude_domains`, `recency`, `country`. `read_web_page` requires `url`; optional `objective`, `forceRefetch` (accepted for compatibility — the custom reader already performs a live fetch).

Filters are best-effort per backend and never silently dropped. `include_domains`/`exclude_domains` are normalized to bare hosts (scheme/userinfo/path/port stripped; a leading `*.`/leading dots/`www.`/trailing dots removed; lowercased; IDN canonicalized to punycode via `domainToASCII`; deduped) and matched suffix-aware, so a domain also matches its subdomains. An invalid entry or more than 20 unique domains is rejected with an error rather than silently ignored, and a domain may not appear in both lists (compared after canonicalization). `recency` (`day`/`week`/`month`/`year`) maps natively to Brave `freshness` and SearXNG `time_range`; domains are post-filtered on result hostnames over a widened candidate pool. `country` (ISO 3166-1 alpha-2, e.g. `de`/`jp`) maps natively to Brave's `country` parameter; SearXNG and DuckDuckGo report it as unsupported because they do not region-filter on this surface (SearXNG's search API has no `country` param, and this backend does not currently expose SearXNG locale/language targeting). DuckDuckGo post-filters domains but reports `recency` as unsupported because its HTML results carry no reliable dates. Every requested filter is reported in the tool result `details.filters[]` as `native` / `post_filter` / `unsupported` with `full` / `partial` / `none` enforcement.

### `web_search` backend `auto` precedence

1. **SearXNG** when `ampiWeb.searxngUrl` / `AMPI_WEB_SEARXNG_URL` is set. JSON API call to the user's own instance; no upstream key. See [SearXNG setup](#searxng-setup).
2. **Brave** when `BRAVE_API_KEY` is set. Free `Data for AI` tier suffices for typical usage. Requests set `text_decorations=false` (clean snippets, no highlight markers) and `result_filter=web` (only the result block the parser consumes).
...
Pin a backend with `ampiWeb.searchBackend` / `AMPI_WEB_SEARCH_BACKEND`: `searxng`, `brave`, or `duckduckgo`. `read_web_page` always uses the custom reader.
...
Inside Pi, `/ampi-config` + the **web** branch is an interactive editor for safe runtime fields (`enabled`, `backend`, `searchBackend`, `searxngUrl`, sidecar booleans/URLs/timers, search/read timeouts, `maxResultBytes`). API keys are env-only; `searxngStartCommand` / `searxngStopCommand` are settings-file only because they spawn arbitrary processes. The "show current config" view reports whether `BRAVE_API_KEY` is set and shows sidecar command-array shape (arg counts), not literal commands.

### Environment variables

The `AMPI_WEB_*` names below are canonical; the legacy `MMR_WEB_*` equivalents are still accepted.

| Variable                                                  | Effect                                                                                                            |
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `AMPI_WEB_ENABLE`                                         | `true`/`false` master switch.                                                                                     |
| `AMPI_WEB_BACKEND`                                        | Compatibility pin; for `web_search` accepts `auto`/`searxng`/`brave`/`duckduckgo`. `read_web_page` ignores.       |
| `AMPI_WEB_SEARCH_BACKEND` / `AMPI_WEB_READER_BACKEND`     | Per-tool override; reader stays on the custom in-process pipeline when network is enabled.                        |
| `AMPI_WEB_SEARXNG_URL` / `AMPI_WEB_SEARXNG_HEALTH_URL`    | SearXNG URL / explicit health-poll URL (default `${searxngUrl}/search?q=ping&format=json`).                       |
| `BRAVE_API_KEY`                                           | Brave Search API key. **Env only.**                                                                               |
| `AMPI_WEB_SEARCH_TIMEOUT_MS` / `AMPI_WEB_READ_TIMEOUT_MS` | Per-call timeouts (default 30 s).                                                                                 |
| `AMPI_WEB_MAX_RESULT_BYTES`                               | Truncation budget (default 200 KB).                                                                               |
| `AMPI_WEB_SEARXNG_MANAGED`                                | Opt-in managed sidecar lifecycle.                                                                                 |
| `AMPI_WEB_SEARXNG_IDLE_TIMEOUT_MS` / `…_START_TIMEOUT_MS` | Sidecar idle stop (default 900 000; `0` disables) / max wait for health pass (default 30 000).                    |

`AMPI_WEB_SEARXNG_START_COMMAND` / `AMPI_WEB_SEARXNG_STOP_COMMAND` are **never** read from the environment — settings-file only — and env versions are rejected with a warning.

API keys are not read from settings files. Key-shaped fields such as `ampiWeb.braveApiKey` are ignored and warned about. Set `BRAVE_API_KEY` in env for the Brave backend.

## Behavior

### SearXNG setup

[SearXNG](https://docs.searxng.org/) is a self-hosted meta-search engine. `ampi-web` calls an instance you control; upstream engines never see your queries directly.
...
SearXNG ships `formats: [html]` by default. `ampi-web` calls the JSON API, so enable JSON in `settings.yml`:
...
Reload, then `export AMPI_WEB_SEARXNG_URL=http://127.0.0.1:8080`. If the instance returns HTML, `ampi-web` raises an actionable error explaining which knob to flip.

### Managed SearXNG sidecar (opt-in)

`ampi-web` can spawn a local instance on demand and stop it when idle:
...
First `web_search` spawns the start command (no shell, args literal); `ampi-web` polls the health URL until 200, capped at `searxngStartTimeoutMs`. Each successful call re-arms the idle timer (`searxngIdleTimeoutMs`, default 15 min; `0` disables). When it fires, the stop command runs; with no stop command, `ampi-web` can only send `SIGTERM` to a still-alive start process. `pi.on('session_shutdown')` also triggers stop. On Pi 0.77.0+ this `session_shutdown` cleanup also runs on `SIGTERM`/`SIGHUP` signal exits (not just clean session swaps or quit), so signal-terminated sessions no longer leak the sidecar.
...
- **Tools stay `gated` in `/ampi-status`.** `AMPI_WEB_ENABLE=true` (or `ampiWeb.enabled=true`) was not set, or the extension did not reload. Restart Pi.
...
Re-exported from `ampi`: `createAmpiWebExtension`, `MMR_WEB_OWNED_TOOLS`, plus the web tool / details types. Canonical catalog: [`../../../docs/public-api.md`](../../../docs/public-api.md).
