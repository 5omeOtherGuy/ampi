...
export interface BackendDecision {
  ...
  /** Stable code used in `/ampi-status` reasons and provider diagnostics. */
  reason: "ok" | "disabled";
  /** Human-readable explanation suitable for `/ampi-status` rows. */
  message: string;
}
 ...
 * - `read_web_page`: ampi-web's custom direct reader only. It requires
...
export function resolveBackend(tool: ResolvedTool, settings: MmrWebSettings): BackendDecision {
  if (!settings.enabled) {
    return {
      reason: "disabled",
      message: "ampi-web network access is disabled (set AMPI_WEB_ENABLE=true or legacy MMR_WEB_ENABLE=true to enable).",
    };
  }
...
export function getSearchBackend(
  ...
  switch (decision.backend) {
    ...
    default:
      ...
      throw new Error(`ampi-web: invalid search backend "${decision.backend}".`);
  }
}
...
export function getReader(
  ...
  if (decision.backend !== "custom") {
    throw new Error(`ampi-web: invalid reader backend "${decision.backend}".`);
  }
