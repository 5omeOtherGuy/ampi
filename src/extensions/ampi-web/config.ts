...
import { parseBoolEnv, readPreferredEnv, type PreferredEnvValue } from "../ampi-core/internal/env.js";
import { isRecord } from "../ampi-core/internal/json.js";

/**
 * Backend selection for `ampi-web`.
 *
...
export interface MmrWebSettings {
  ...
  searxngUrl?: string;
  /**
   * Opt-in: have `ampi-web` start a local SearXNG instance on demand and
   ...
   * file only (project-local settings are ignored). When omitted, `ampi-web`
...
function selectWebSettingsBlock(value: Record<string, unknown>): { raw: Record<string, unknown>; rootKey: string } | undefined {
  const ampiDirect = isRecord(value.ampiWeb) ? value.ampiWeb : undefined;
  if (ampiDirect) return { raw: ampiDirect, rootKey: "ampiWeb" };
  const ampiNested = isRecord(value.ampi) && isRecord((value.ampi as Record<string, unknown>).web)
    ? ((value.ampi as Record<string, unknown>).web as Record<string, unknown>)
    : undefined;
  if (ampiNested) return { raw: ampiNested, rootKey: "ampi.web" };
  const legacyDirect = isRecord(value.mmrWeb) ? value.mmrWeb : undefined;
  if (legacyDirect) return { raw: legacyDirect, rootKey: "mmrWeb" };
  const legacyNested = isRecord(value.mmr) && isRecord((value.mmr as Record<string, unknown>).web)
    ? ((value.mmr as Record<string, unknown>).web as Record<string, unknown>)
    : undefined;
  return legacyNested ? { raw: legacyNested, rootKey: "mmr.web" } : undefined;
}
...
function extractMmrWebSettings(value: unknown): ExtractedMmrWebSettings | undefined {
  ...
  const selected = selectWebSettingsBlock(value);
  ...
  const hasJinaApiKey =
    typeof raw.jinaApiKey === "string";
  const hasBraveApiKey = typeof raw.braveApiKey === "string";
  return {
    rootKey,
    values: out,
    hasJinaApiKey,
...
function parseBackendEnv(source: PreferredEnvValue | undefined, warnings: string[]): MmrWebBackend | undefined {
  return parseNamedBackendEnv(source?.name ?? "AMPI_WEB_BACKEND", source?.value, warnings);
}
...
function parseNamedBackendEnv(
  ...
  warnings.push(
    `Ignoring ${name}="${value}": expected one of ${MMR_WEB_BACKENDS.join(", ")}; ampi-web no longer supports Jina.`,
  );
  return undefined;
}

/**
 * Load ampi-web settings from the standard MMR settings files (global home +
...
export function loadMmrWebSettings(
  ...
  for (const filePath of files) {
    ...
    if (filePath !== globalSettingsPath &&
      ...
      warnings.push(
        `Ignoring ${extracted.rootKey}.searxngStartCommand/searxngStopCommand in ${filePath}: the managed-SearXNG start/stop commands spawn arbitrary processes and are honored only from your global settings file (~/.pi/agent/settings.json), not from project-local settings.`,
      );
    ...
    if (extracted.hasJinaApiKey) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.jinaApiKey in ${filePath}: ampi-web no longer uses Jina. Remove this settings-file secret and set BRAVE_API_KEY in the environment for web_search.`,
      );
    }
    if (extracted.hasBraveApiKey) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.braveApiKey in ${filePath}: the Brave Search API key must come from the BRAVE_API_KEY environment variable, not from settings files (which are commonly committed or synced).`,
      );
    }
    if (extracted.invalidBackend !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.backend="${extracted.invalidBackend}" in ${filePath}: expected one of ${MMR_WEB_BACKENDS.join(", ")}; ampi-web no longer supports Jina.`,
      );
    }
    if (extracted.invalidSearchBackend !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.searchBackend="${extracted.invalidSearchBackend}" in ${filePath}: expected one of ${MMR_WEB_BACKENDS.join(", ")}; ampi-web no longer supports Jina.`,
      );
    }
    if (extracted.invalidReaderBackend !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.readerBackend="${extracted.invalidReaderBackend}" in ${filePath}: expected one of ${MMR_WEB_BACKENDS.join(", ")}; ampi-web no longer supports Jina.`,
      );
    }
    if (extracted.invalidSearxngUrl !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.searxngUrl="${extracted.invalidSearxngUrl}" in ${filePath}: expected a http(s) URL.`,
      );
    }
    if (extracted.invalidSearxngHealthUrl !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.searxngHealthUrl="${extracted.invalidSearxngHealthUrl}" in ${filePath}: expected a http(s) URL.`,
      );
    }
    if (extracted.invalidSearxngStartCommand !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.searxngStartCommand=${extracted.invalidSearxngStartCommand} in ${filePath}: expected a non-empty array of strings (e.g. ["docker", "compose", "-f", "./searxng.yml", "up", "-d"]).`,
      );
    }
    if (extracted.invalidSearxngStopCommand !== undefined) {
      warnings.push(
        `Ignoring ${extracted.rootKey}.searxngStopCommand=${extracted.invalidSearxngStopCommand} in ${filePath}: expected a non-empty array of strings.`,
      );
    }
  }

  const enableEnv = parseBoolEnv(readPreferredEnv(env, "AMPI_WEB_ENABLE", "MMR_WEB_ENABLE")?.value);
  if (enableEnv !== undefined) merged.enabled = enableEnv;
  const managedEnv = parseBoolEnv(readPreferredEnv(env, "AMPI_WEB_SEARXNG_MANAGED", "MMR_WEB_SEARXNG_MANAGED")?.value);
  if (managedEnv !== undefined) merged.searxngManaged = managedEnv;

  const searxngHealthUrlEnv = readPreferredEnv(env, "AMPI_WEB_SEARXNG_HEALTH_URL", "MMR_WEB_SEARXNG_HEALTH_URL");
  if (searxngHealthUrlEnv) {
    const trimmed = searxngHealthUrlEnv.value.trim();
    if (trimmed) {
      try {
        const parsed = new URL(trimmed);
        if (parsed.protocol === "http:" || parsed.protocol === "https:") {
          ...
          warnings.push(
            `Ignoring ${searxngHealthUrlEnv.name}="${searxngHealthUrlEnv.value}": expected a http(s) URL.`,
          );
        }
      } catch {
        warnings.push(
  ...
  const idleSource = readPreferredEnv(env, "AMPI_WEB_SEARXNG_IDLE_TIMEOUT_MS", "MMR_WEB_SEARXNG_IDLE_TIMEOUT_MS");
  const idleEnv = parsePositiveInt(idleSource?.value);
  if (idleEnv !== undefined) merged.searxngIdleTimeoutMs = idleEnv;
  // Also accept 0 (idle-stop disabled) via env.
  if (idleSource?.value === "0") merged.searxngIdleTimeoutMs = 0;
  const startTimeoutEnv = parsePositiveInt(readPreferredEnv(env, "AMPI_WEB_SEARXNG_START_TIMEOUT_MS", "MMR_WEB_SEARXNG_START_TIMEOUT_MS")?.value);
  if (startTimeoutEnv !== undefined) merged.searxngStartTimeoutMs = startTimeoutEnv;

  const startCommandEnv = readPreferredEnv(env, "AMPI_WEB_SEARXNG_START_COMMAND", "MMR_WEB_SEARXNG_START_COMMAND");
  if (startCommandEnv) {
    warnings.push(
      `Ignoring ${startCommandEnv.name}: the managed-SearXNG start/stop commands must come from the ampiWeb.searxngStartCommand / ampiWeb.searxngStopCommand fields in your settings file (settings.json), not from environment variables.`,
    );
  }
  const stopCommandEnv = readPreferredEnv(env, "AMPI_WEB_SEARXNG_STOP_COMMAND", "MMR_WEB_SEARXNG_STOP_COMMAND");
  if (stopCommandEnv) {
    warnings.push(
      `Ignoring ${stopCommandEnv.name}: the managed-SearXNG start/stop commands must come from the ampiWeb.searxngStartCommand / ampiWeb.searxngStopCommand fields in your settings file (settings.json), not from environment variables.`,
    );
  }
  if (typeof env.BRAVE_API_KEY === "string" && env.BRAVE_API_KEY.trim()) {
    merged.braveApiKey = env.BRAVE_API_KEY.trim();
  }
  const backendEnv = parseBackendEnv(readPreferredEnv(env, "AMPI_WEB_BACKEND", "MMR_WEB_BACKEND"), warnings);
  if (backendEnv !== undefined) merged.backend = backendEnv;
  const searchBackendSource = readPreferredEnv(env, "AMPI_WEB_SEARCH_BACKEND", "MMR_WEB_SEARCH_BACKEND");
  const searchBackendEnv = parseNamedBackendEnv(
    searchBackendSource?.name ?? "AMPI_WEB_SEARCH_BACKEND",
    searchBackendSource?.value,
    warnings,
  );
  if (searchBackendEnv !== undefined) merged.searchBackend = searchBackendEnv;
  const readerBackendSource = readPreferredEnv(env, "AMPI_WEB_READER_BACKEND", "MMR_WEB_READER_BACKEND");
  const readerBackendEnv = parseNamedBackendEnv(
    readerBackendSource?.name ?? "AMPI_WEB_READER_BACKEND",
    readerBackendSource?.value,
    warnings,
  );
  if (readerBackendEnv !== undefined) merged.readerBackend = readerBackendEnv;
  const searxngUrlEnv = readPreferredEnv(env, "AMPI_WEB_SEARXNG_URL", "MMR_WEB_SEARXNG_URL");
  if (searxngUrlEnv) {
    const trimmed = searxngUrlEnv.value.trim();
    if (trimmed) {
      try {
        const parsed = new URL(trimmed);
        if (parsed.protocol === "http:" || parsed.protocol === "https:") {
          ...
          warnings.push(
            `Ignoring ${searxngUrlEnv.name}="${searxngUrlEnv.value}": expected a http(s) URL.`,
          );
        }
      } catch {
        warnings.push(
  ...
  const searchTimeout = parsePositiveInt(readPreferredEnv(env, "AMPI_WEB_SEARCH_TIMEOUT_MS", "MMR_WEB_SEARCH_TIMEOUT_MS")?.value);
  if (searchTimeout) merged.searchTimeoutMs = searchTimeout;
  const readTimeout = parsePositiveInt(readPreferredEnv(env, "AMPI_WEB_READ_TIMEOUT_MS", "MMR_WEB_READ_TIMEOUT_MS")?.value);
  if (readTimeout) merged.readTimeoutMs = readTimeout;
  const maxBytes = parsePositiveInt(readPreferredEnv(env, "AMPI_WEB_MAX_RESULT_BYTES", "MMR_WEB_MAX_RESULT_BYTES")?.value);
