...
import { registerMmrConfigFlowSection } from "../ampi-core/config-flow-registry.js";
...
import { registerAmpiWebToolSourcePath } from "./tool-ownership.js";
...
// Recording our entrypoint path here lets `ampi-core` Free mode and
// worker gating confirm ownership of `web_search` /
...
const AMPI_WEB_EXTENSION_ENTRYPOINT_PATH = fileURLToPath(import.meta.url);
registerAmpiOwnedExtensionPath(AMPI_WEB_EXTENSION_ENTRYPOINT_PATH);
registerAmpiWebToolSourcePath(AMPI_WEB_EXTENSION_ENTRYPOINT_PATH);

// Own the `web` section of `/ampi-config` by registering it with ampi-core,
// rather than ampi-core importing this flow (inverts the core->sibling import).
...
/**
 * Build a Pi extension factory for ampi-web with optional test seams.
 *
...
export function createMmrWebExtension(overrides: MmrWebFactoryOverrides = {}) {
  return function mmrWebExtension(pi: ExtensionAPI): void {
    ...
    // settings mid-process would desync the live `ampi-web` provider gate
    ...
    // restarting the Pi process. See src/extensions/ampi-web/README.md.
...
export const createAmpiWebExtension = createMmrWebExtension;

const ampiWebExtension = createMmrWebExtension();

export default ampiWebExtension;
