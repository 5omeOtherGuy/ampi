...
This roadmap covers the `ampi-web` extension: network-backed tools (off by
...
Shipped, disabled by default; opt-in via `AMPI_WEB_ENABLE=true` (legacy `MMR_WEB_ENABLE` still accepted).

### Search backends (pluggable)

- ✅ Pluggable `SearchBackend` interface under `src/extensions/ampi-web/search/`
...
- ✅ **SearXNG** backend (`search/searxng.ts`) — user-controlled instance,
  no upstream key. URL via `ampiWeb.searxngUrl` or `AMPI_WEB_SEARXNG_URL`.
...
- ✅ Explicit per-tool overrides via `ampiWeb.searchBackend` /
  `AMPI_WEB_SEARCH_BACKEND`.
...
- ✅ Opt-in via `ampiWeb.searxngManaged=true` AND a non-empty
  `ampiWeb.searxngStartCommand`.
...
- ✅ ampi tool provider and `ampi-web` feature-gate provider that report
  gated/active state in `/ampi-status`.
...
- ✅ `/ampi-config web` lets users edit `enabled`, backend pins,
...
- `ampi-core` tool registry, feature gates, network/privacy policy.
...
- **Cross-process sidecar coordination.** Today the managed SearXNG sidecar
  ...
  `<cwd>/.pi/data/ampi-web/searxng.lock` recording `{pid, startedAt, url}`
...
- **Per-call telemetry / diagnostics** surfaced through `/ampi-status`
...
- Off by default; explicit opt-in via `AMPI_WEB_ENABLE` (or a new gate,
