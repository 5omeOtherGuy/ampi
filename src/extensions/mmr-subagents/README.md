...
- `selectFinderWorkerModel` is derived from the `finder` profile. If no route matches, `--model` is omitted and the worker inherits Pi's default.
...
- `MmrSubagentsCapabilities` — per-tool flags (`finder`, `oracle`, `Task`, `librarian`); booleans or zero-argument predicates (so `librarian` can reflect live mmr-web state).

Tool-specific (`create<X>Tool(deps?)` / `register<X>Tool(pi, deps?)` plus constants, schemas, prompt helpers, types):

- **Finder.** `FINDER_TOOL_NAME`, `FINDER_WORKER_TOOLS`, `FINDER_DEFAULT_MODEL_PREFERENCES`, `FINDER_PROMPT_SNIPPET`, `FINDER_PROMPT_GUIDELINES`, `FINDER_DESCRIPTION`, `FINDER_PARAMETERS_SCHEMA`, `FINDER_PROGRESS_PLACEHOLDER`, `buildFinderWorkerSystemPrompt(cwd)`, `selectFinderWorkerModel(availableModels, preferences?)`. Types: `FinderParams`, `FinderDetails`, `FinderToolDeps`.
...
- **Librarian.** `LIBRARIAN_TOOL_NAME`, `LIBRARIAN_SUBAGENT_PROFILE_NAME`, `LIBRARIAN_WORKER_TOOLS`, prompt/schema constants, `buildLibrarianWorkerSystemPrompt(cwd)`, `isLibrarianWebToolPrerequisiteRegistered(pi)`, `isLibrarianWebToolPrerequisiteActive(pi)`, `LIBRARIAN_GATING_REASON`, `MmrLibrarianContextWindowError`. Types: `LibrarianParams`, `LibrarianDetails`, `LibrarianStatus`, `LibrarianToolDeps`, `ResolveLibrarianInvocationInput`. Params `{ query, context? }`.
