...
/**
 * ampi-debug (formerly mmr-debug) — opt-in live request/response capture for debugging model behavior.
 *
 ...
 *
 * It is fully inert unless `AMPI_DEBUG_CAPTURE_FILE` (or legacy
 * `MMR_DEBUG_CAPTURE_FILE`) is set: with no env var it
 ...
 *
 * Set `AMPI_DEBUG_CAPTURE_FULL=1`/`true` (or legacy
 * `MMR_DEBUG_CAPTURE_FULL`) to also dump the entire raw request payload (all
 * conversation messages) on each "request" record.
 *
...
const AMPI_CAPTURE_FILE_ENV = "AMPI_DEBUG_CAPTURE_FILE";
const CAPTURE_FILE_ENV = "MMR_DEBUG_CAPTURE_FILE";
const AMPI_CAPTURE_FULL_ENV = "AMPI_DEBUG_CAPTURE_FULL";
...
export function createMmrDebugExtension() {
  return function mmrDebugExtension(pi: ExtensionAPI): void {
    const capturePath = readPreferredEnv(process.env, AMPI_CAPTURE_FILE_ENV, CAPTURE_FILE_ENV)?.value;
    ...
    const captureFullPayload = parseBoolEnv(
      readPreferredEnv(process.env, AMPI_CAPTURE_FULL_ENV, CAPTURE_FULL_ENV)?.value,
    ) ?? false;
