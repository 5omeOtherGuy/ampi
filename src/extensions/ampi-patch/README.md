...
| --- | --- | --- | --- |
| On | `apply_patch` | none | `/ampi-status`, tool result `details` |
...
Stable re-exports from `ampi`: `registerAmpiPatchProviders`, `ApplyPatchError`. Canonical catalog: [`../../../docs/public-api.md`](../../../docs/public-api.md).

## Developer notes

- `ampi-patch` was split out of the former `ampi-toolbox`, which is now a deprecated compatibility shim that re-exports this extension (and `ampi-tasks`).
