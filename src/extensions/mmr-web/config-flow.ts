...
function formatCurrentConfig(settings: MmrWebSettings, filePath: string): string {
  ...
  return [
    `Current mmr-web configuration (persists to ${filePath}):`,
...
/**
 * Interactive editor for `mmr-web` settings. Surfaced through the
 * `/mmr-config` slash command's "web" branch: pick a field, pick the new
...
export async function runMmrWebConfigFlow(ctx: ExtensionContext): Promise<void> {
  if (ctx.hasUI === false) {
    ctx.ui.notify("mmr-web configuration requires an interactive UI.", "warning");
    return;
  }
  ...
  const choice = await ctx.ui.select("mmr-web config: what do you want to set?", [
    "show current config",
  ...
  try {
    ...
    ctx.ui.notify(`Failed to write mmr-web config to ${filePath}: ${message}`, "error");
    return;
  }

  ctx.ui.notify(
    `Saved mmr-web config to ${filePath}.\nRestart Pi for the change to take effect; mmr-web samples settings once at extension load.`,
    "info",
  );
}
