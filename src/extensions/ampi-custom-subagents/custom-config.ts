...
function readAgentsBlock(value: unknown): { agents: Record<string, unknown>; rootKey: string } | undefined {
  if (!isRecord(value)) return undefined;
  const ampiFlat = isRecord(value.ampiSubagents) ? value.ampiSubagents : undefined;
  const ampiNested = isRecord(value.ampi) && isRecord((value.ampi as Record<string, unknown>).subagents)
    ? ((value.ampi as Record<string, unknown>).subagents as Record<string, unknown>)
    : undefined;
  const legacyFlat = isRecord(value.mmrSubagents) ? value.mmrSubagents : undefined;
  const legacyNested = isRecord(value.mmr) && isRecord((value.mmr as Record<string, unknown>).subagents)
    ? ((value.mmr as Record<string, unknown>).subagents as Record<string, unknown>)
    : undefined;
  const selected = ampiFlat
    ? { subagents: ampiFlat, rootKey: "ampiSubagents" }
    : ampiNested
      ? { subagents: ampiNested, rootKey: "ampi.subagents" }
      : legacyFlat
        ? { subagents: legacyFlat, rootKey: "mmrSubagents" }
        : legacyNested
          ? { subagents: legacyNested, rootKey: "mmr.subagents" }
          : undefined;
  ...
  return agents ? { agents, rootKey: selected.rootKey } : undefined;
}
...
function normalizeModeScope(
  value: unknown,
  ctx: { id: string; filePath: string; rootKey: string; warnings: string[] },
): MmrCustomSubagentModeScope | undefined {
  if (value === "allLocked") return "allLocked";
  if (!Array.isArray(value)) {
    ctx.warnings.push(
      `Ignoring ${ctx.rootKey}.custom.agents.${ctx.id}.modes in ${ctx.filePath}: expected "allLocked" or an array of locked mode keys.`,
    );
    return undefined;
  }
...
function normalizeProjectScope(
  ...
  for (const entry of normalizeStringList(value)) {
    if (!path.isAbsolute(entry)) {
      ctx.warnings.push(
        `Ignoring relative project path "${entry}" for ${ctx.rootKey}.custom.agents.${ctx.id} in ${ctx.filePath}: global "projects" entries must be absolute cwd paths.`,
      );
      continue;
    }
...
function parseRecord(
  ...
  ctx: { filePath: string; rootKey: string; warnings: string[] },
): MmrCustomSubagentRecord | undefined {
  if (!isRecord(raw)) {
    ctx.warnings.push(
      `Ignoring ${ctx.rootKey}.custom.agents.${id} in ${ctx.filePath}: expected an object.`,
    );
    return undefined;
  }
  ...
  if (fileRaw.length === 0) {
    ctx.warnings.push(
      `Ignoring ${ctx.rootKey}.custom.agents.${id} in ${ctx.filePath}: missing source.file.`,
    );
    return undefined;
  }
  if (isUnsafeRelativeSourceFile(fileRaw)) {
    ctx.warnings.push(
      `Ignoring ${ctx.rootKey}.custom.agents.${id} in ${ctx.filePath}: source.file must be a relative path inside the Pi-owned subagent root (no ".." segments, no absolute paths).`,
    );
    return undefined;
  }

  const modes = normalizeModeScope(raw.modes, { id, filePath: ctx.filePath, rootKey: ctx.rootKey, warnings: ctx.warnings });
  ...
  if (!CUSTOM_SUBAGENT_TOOL_NAME_PATTERN.test(toolName)) {
    ctx.warnings.push(
      `Ignoring ${ctx.rootKey}.custom.agents.${id} in ${ctx.filePath}: toolName must match "${MMR_CUSTOM_SUBAGENT_TOOL_PREFIX}<slug>" (lowercase letters, digits, underscores; up to 60 slug chars).`,
    );
    return undefined;
  }
  ...
  const projects = normalizeProjectScope(raw.projects, { id, filePath: ctx.filePath, rootKey: ctx.rootKey, warnings: ctx.warnings });
...
export function loadMmrSubagentsConfig(args: LoadMmrSubagentsConfigArgs): LoadedMmrSubagentsConfig {
  ...
  for (const { path: filePath, layer } of settingsFilePaths(args.cwd, homeDir)) {
    ...
    const selected = readAgentsBlock(parsed);
    if (!selected) continue;
    filesRead.push(filePath);
    for (const [id, value] of Object.entries(selected.agents)) {
      if (typeof id !== "string" || id.length === 0 || RESERVED_AGENT_IDS.has(id)) continue;
      const record = parseRecord(id, value, layer, { filePath, rootKey: selected.rootKey, warnings });
...
/**
 * Insert or replace a single `ampiSubagents.custom.agents.<id>` record in a Pi
 * settings file. Preserves an existing flat/nested `ampi` or legacy `mmr`
 * layout, all unrelated keys, and other agent records. When `record` is
 * `undefined` the agent id is removed. Returns the file path.
 */
export function writeMmrSubagentsConfigRecord(
  ...
  return rewriteJsonSettingsFile(filePath, (existing) => {
    ...
    const ampiFlat = isRecord(root.ampiSubagents) ? { ...root.ampiSubagents } : undefined;
    const ampiBlock = isRecord(root.ampi) ? { ...root.ampi } : undefined;
    const ampiNested = ampiBlock && isRecord(ampiBlock.subagents) ? { ...ampiBlock.subagents } : undefined;

    const legacyFlat = isRecord(root.mmrSubagents) ? { ...root.mmrSubagents } : undefined;
    const mmrBlock = isRecord(root.mmr) ? { ...root.mmr } : undefined;
    const legacyNested = mmrBlock && isRecord(mmrBlock.subagents) ? { ...mmrBlock.subagents } : undefined;

    const target = ampiFlat
      ? { kind: "flat" as const, flatKey: "ampiSubagents", subagents: ampiFlat }
      : ampiNested
        ? { kind: "nested" as const, rootKey: "ampi", block: ampiBlock ?? {}, subagents: ampiNested }
        : legacyFlat
          ? { kind: "flat" as const, flatKey: "mmrSubagents", subagents: legacyFlat }
          : legacyNested
            ? { kind: "nested" as const, rootKey: "mmr", block: mmrBlock ?? {}, subagents: legacyNested }
            : { kind: "flat" as const, flatKey: "ampiSubagents", subagents: {} };

    const subagents: Record<string, unknown> = target.subagents;
    ...
    if (target.kind === "nested") {
      ...
      nextBlock.subagents = subagents;
      root[target.rootKey] = nextBlock;
    } else {
      root[target.flatKey] = subagents;
    }
