...
// Own the custom-subagent setup/import section of `/ampi-config` by registering
// it with ampi-core, rather than ampi-core importing this flow.
registerMmrConfigFlowSection({
  id: "ampi-custom-subagents",
...
export function createMmrCustomSubagentsExtension(overrides: MmrCustomSubagentsFactoryOverrides = {}) {
  return function ampiCustomSubagentsExtension(pi: ExtensionAPI): void {
...
export const createAmpiCustomSubagentsExtension = createMmrCustomSubagentsExtension;
...
function maybeNotifyLegacyClaudeMigration(ctx: ExtensionContext): void {
  try {
    ...
    ctx.ui.notify(
      "Claude-style agents are no longer auto-loaded by ampi. Run /ampi-config → \"subagent (setup/import custom)\" to review and enable selected agents.",
      "info",
    );
  } catch {
...
const ampiCustomSubagentsExtension = createMmrCustomSubagentsExtension();

export default ampiCustomSubagentsExtension;
