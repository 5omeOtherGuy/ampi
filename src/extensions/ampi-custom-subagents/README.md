...
Package overview: [`../../../README.md`](../../../README.md). Public API: [`../../../docs/public-api.md`](../../../docs/public-api.md). Sibling worker extension: [`../ampi-workers/README.md`](../ampi-workers/README.md). Framework boundary: [`../../../docs/subagent-framework.md`](../../../docs/subagent-framework.md).
...
`ampi-custom-subagents` ships loaded but inert. It was extracted from `ampi-workers` and is registered in `package.json` under `pi.extensions`.
...
| `/ampi-config` → "subagent (setup/import custom)" | config flow | Scan candidates, recommend a least-privilege config, enable/import a subagent. |
...
Enablement is config-driven and is the privilege boundary. Records live under `ampiSubagents.custom.agents` (legacy `mmrSubagents` still read) and are merged global-then-project, with the project layer overriding by id.
...
- **My subagent tool is missing.** Discovery is not activation. Confirm an enabled `ampiSubagents.custom.agents.<id>` record references the file and that the source Markdown is valid and in a Pi-owned root. The feature gate stays disabled until at least one registers.
...
- **Legacy agents are not loading.** `.claude/agents` is import-only. Run `/ampi-config` → "subagent (setup/import custom)" to import them into a Pi-owned root and write an enabled record.
...
- `createAmpiCustomSubagentsExtension(overrides?)`, `MmrCustomSubagentsFactoryOverrides`.
- `createAmpiCustomSubagentsFeatureGateProvider(...)`, `createAmpiCustomSubagentsToolProvider(...)`, `MmrCustomSubagentsCapabilities`.
- `AMPI_CUSTOM_SUBAGENTS_FEATURE_GATE`, `AMPI_CUSTOM_SUBAGENTS_PROVIDER_NAME`.
...
- Custom workers route through the shared `ampi-workers` framework (profile + prompt builder + child-worker runner). Do not hard-code an independent runner or prompt path here.
- The tool provider claims only the reserved `sa__*` namespace and returns `undefined` for everything else, so it never shadows `ampi-core`, `ampi-workers`, or user aliases.
